...
void loop() {
  ...
  int currentX = positions[0][0];
  int currentY = positions[0][1];

  for (int i = 1; i < 4; i++) {
    int nextX = positions[i][0];
    int nextY = positions[i][1];
    moveToNextPoint(currentX, currentY, nextX, nextY);
    currentX = nextX;
    currentY = nextY;
  }
